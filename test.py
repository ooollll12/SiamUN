...
parser = argparse.ArgumentParser(description='Test SiamUN')
...
def track_vos(model, video, hp=None, mask_enable=False, refine_enable=False, mot_enable=False, device='cpu'):
    ...
    if args.save_mask:
        video_path = join('test', args.dataset, 'SiamUN', video['name'])
        if not isdir(video_path): makedirs(video_path)
...
def main():
    ...
    model = SiamUN(anchors=cfg['anchors']).to(device)

    if args.resume:
        assert isfile(args.resume), '{} is not a valid file'.format(args.resume)
        pretrained_dict = torch.load(args.resume, map_location=device)  # 加载权重到指定设备
        model.load_state_dict(pretrained_dict)  # 加载状态字典
    model.eval()
